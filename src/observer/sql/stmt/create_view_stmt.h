...
public:
  ...
  CreateViewStmt(const std::string &view_name, const std::vector<std::string> &col_names, const std::string &sql_str,
    std::vector<AttrInfoSqlNode> &attr_infos, std::vector<ViewMetaInfo> &view_meta_infos)
      : view_name_(view_name), col_names_(col_names), sql_str_(sql_str), attr_infos_(attr_infos), view_meta_infos_(view_meta_infos) {}
  ...
  const std::vector<ViewMetaInfo> &view_meta_infos() const { return view_meta_infos_; }
